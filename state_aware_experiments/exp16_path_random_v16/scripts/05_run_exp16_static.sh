#!/usr/bin/env bash
# exp16c: PATH_STATIC sanity check — collision-free precomputed path assignment.
# Each flow is locked to exactly one agg→core uplink with no sharing (tornado TM
# has a perfect 1:1 assignment).  Expected FCT slowdown ≈ 1.001× optimal.
...
echo ""
echo "PATH_STATIC runs complete. Re-run:"
