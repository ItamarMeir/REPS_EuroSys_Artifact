...
Every new experiment is a sibling directory under `state_aware_experiments/` named `expNN_short_name/` (NN = next integer), with this internal shape:
...
Open [`state_aware_experiments/README.md`](README.md), add a new row to the lineage table, and add a one-line takeaway. Keep the table chronological; don't re-order.
...
for d in plots data scripts; do test -d state_aware_experiments/$EXP/$d || echo "MISSING: $d/"; done
test -f state_aware_experiments/$EXP/README.md       || echo "MISSING: README.md"
test -f state_aware_experiments/$EXP/runs.tar.gz     || echo "WARNING: no runs.tar.gz"

# 2. README links to relative plot paths (no /tmp, no absolute)
grep -E "^!\[.*\]\((/|/tmp)" state_aware_experiments/$EXP/README.md \
  && echo "BUG: absolute or /tmp image refs in README"

# 3. Scripts portable
grep -l "/tmp/" state_aware_experiments/$EXP/scripts/* \
  && echo "BUG: scripts reference /tmp"

# 4. Master README mentions the new experiment
grep -q "$EXP" state_aware_experiments/README.md || echo "TODO: update master README"
```
