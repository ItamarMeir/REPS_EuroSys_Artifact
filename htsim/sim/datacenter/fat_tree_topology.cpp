...
bool FatTreeTopology::skip_asy = false;
...
double FatTreeTopology::_failed_link_ratio = 0.5;
...
void FatTreeTopology::init_network(){
    ...
    if (_num_failed_links > 0) {
        fail_every = NTOR / _num_failed_links;
    }
    ...
    for (uint32_t tor = 0; tor < NTOR; tor++) {
        ...
        for (uint32_t l = 0; l < link_bundles; l++) {
            ...
            for (uint32_t b = 0; b < _bundlesize[TOR_TIER]; b++) {
                ...
                pipes_nlp_ns[tor][srv][b]->full_name = ("Pipe-LS" + ntoa(tor)  + "->DST" + ntoa(srv) + "(" + ntoa(b) + ")");
                ...
                pipes_ns_nlp[srv][tor][b]->full_name = ("Pipe-SRC" + ntoa(srv) + "->LS" + ntoa(tor) + "(" + ntoa(b) + ")");
    ...
    for (uint32_t tor = 0; tor < NTOR; tor++) {
        bool can_fail = false;
        if (fail_every > 0 && tor % fail_every == 0) {
            can_fail = true;
        }
        ...
        for (uint32_t agg=agg_min; agg<=agg_max; agg++){
            for (uint32_t b = 0; b < _bundlesize[AGG_TIER]; b++) {
                ...
                if (/* _tiers == 2 &&  */(agg - agg_min) < _num_failed_links && false && degraded_cables < _num_failed_links){
                    queues_nup_nlp[agg][tor][b] = alloc_queue(queueLogger, _downlink_speeds[AGG_TIER]  * _failed_link_ratio, _queue_down[AGG_TIER], DOWNLINK, AGG_TIER,false,true);
                    ...
                    if (disable_links_count < disable_links_tot && false) {
                        disable_links_count++;
                    } else {
                ...
                pipes_nup_nlp[agg][tor][b]->full_name = ("Pipe-US" + ntoa(agg) + "->LS" + ntoa(tor) + "(" + ntoa(b) + ")");
                ...
                if (can_fail &&/* _tiers == 2 &&  */(agg - agg_min) < _num_failed_links && degraded_cables < _num_failed_links){
                ...
                pipes_nlp_nup[tor][agg][b]->full_name = ("Pipe-LS" + ntoa(tor) + "->US" + ntoa(agg) + "(" + ntoa(b) + ")");
                //if (logfile) logfile->writeName(*(pipes_nlp_nup[tor][agg]));
                printf("Creating New Pipe %d %d %d: %s\n", tor, agg, b, pipes_nlp_nup[tor][agg][b]->full_name.c_str());
        ...
        for (uint32_t agg = 0; agg < NAGG; agg++) {
            ...
            for (uint32_t l = 0; l < _radix_up[AGG_TIER]/_bundlesize[CORE_TIER]; l++) {
                ...
                for (uint32_t b = 0; b < _bundlesize[CORE_TIER]; b++) {
                    ...
                    pipes_nup_nc[agg][core][b]->full_name = ("Pipe-US" + ntoa(agg) + "->CS" + ntoa(core) + "(" + ntoa(b) + ")");
                    ...
                    pipes_nc_nup[core][agg][b]->full_name = ("Pipe-CS" + ntoa(core) + "->US" + ntoa(agg) + "(" + ntoa(b) + ")");
    ...
    printf("cable and switch failure lists %d %d...\n", FAILURE_GENERATOR->use_list_cable_to_fail, FAILURE_GENERATOR->use_list_switch_to_fail);

    if (FAILURE_GENERATOR->use_list_cable_to_fail && FAILURE_GENERATOR->use_list_switch_to_fail) {
        return;
    }

    if (!FAILURE_GENERATOR->use_list_cable_to_fail && !FAILURE_GENERATOR->use_list_switch_to_fail) {
        FAILURE_GENERATOR->need_to_save = true;
    }

    printf("Creating2 New Pipe: %s\n", pipes_nlp_nup[0][0][0]->full_name.c_str());


    printf("Reaching 1\n");
    // Proper for loop to iterate and print each element
    if (!FAILURE_GENERATOR->use_list_cable_to_fail) {
        bool stop_fail = false;
        for (int i = 0; i < pipes_nlp_nup.size(); ++i) {
            int number_to_fail = round(FAILURE_GENERATOR->cable_fail_max_percent * pipes_nlp_nup[i].size());
            int failed_so_far = 0;
            for (int j = 0; j < pipes_nlp_nup[i].size(); ++j) {
                for (int k = 0; k < pipes_nlp_nup[i][j].size(); ++k) {

                    
                    
                    //printf("I %d - J %d - K %d - To Fail %d - Percentage %f\n", i, j, k, number_to_fail, FAILURE_GENERATOR->cable_fail_max_percent);

                    if (failed_so_far < number_to_fail) {
                        failed_so_far++;
                        /* printf("Size is %d %d %d\n", pipes_nlp_nup.size(), pipes_nlp_nup[i].size(), pipes_nlp_nup[i][j].size());
                        fflush(stdout);
                        printf("1) %d\n", pipes_nlp_nup[i][j][k]->from);
                        fflush(stdout);*/
                        printf("Cable to fail: %s (%d) - %s --- %dto%d\n", pipes_nlp_nup[i][j][k]->nodename().c_str(), pipes_nlp_nup[i][j][k]->getID(), pipes_nlp_nup[i][j][k]->full_name.c_str(), i,j); 
                        FAILURE_GENERATOR->_list_cables_to_fail.push_back(pipes_nlp_nup[i][j][k]->getID());
                        FAILURE_GENERATOR->use_list_cable_to_fail = true;
                        FAILURE_GENERATOR->use_list_switch_to_fail = true;
                    }
                }
            }
        }
    ...
    printf("Reaching 2 %d %d\n", FAILURE_GENERATOR->use_list_switch_to_fail, FAILURE_GENERATOR->_list_switch_to_fail.size());
    if (!FAILURE_GENERATOR->use_list_switch_to_fail) {
        bool stop_fail = false;
        for (int i = 0; i < switches_up.size(); ++i) {
            if ((rand() % 100) < (FAILURE_GENERATOR->switch_fail_max_percent*100)) {
                if (stop_fail) {
                    continue;
                }
                FAILURE_GENERATOR->_list_switch_to_fail.push_back(switches_up[i]->getUniqueID());
                printf("Switch to fail: %s (%d)\n", switches_up[i]->nodename().c_str(), switches_up[i]->getUniqueID());
                FAILURE_GENERATOR->use_list_switch_to_fail = true;
                FAILURE_GENERATOR->use_list_cable_to_fail = true;
                if (FAILURE_GENERATOR->cable_fail_max_percent == 1) {
                    stop_fail = true;
                }
            }
        }
