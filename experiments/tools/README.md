...
`-log_reps_state` (see `experiments/RUNNING_EXPERIMENTS.md`) logs one
...
> **Schema versions.** `fifo` was appended after the first traces were captured.
> The viewer accepts both widths: a 17-column file simply carries no FIFO
> information (rendered as "not instrumented"), and every column before `fifo`
> is byte-identical between the two. Traces produced before this column exists
> stay readable — no need to re-run old experiments.
...
| `slots` | `\|`-joined `value:isValid:lifetime` triples, one per buffer slot, index == slot index. Only FREEZING / FREEZING_PXR drive this buffer; all-zero for every other algorithm. |
| `fifo` | contents of the **unbounded** `_next_pathid` FIFO (`uec.h:947`), front first — the EV buffer plain REPS / MIXED recycle through (`push_back` on a clean ACK, `pop_front` on reuse). `-` means the algorithm does not use the FIFO at all; an empty field means it does but the FIFO is currently empty. Because the list is genuinely unbounded, at most 128 entries are logged and a trailing `+N` token counts those elided. |

Note the two buffers are disjoint: FREEZING uses the bounded circular buffer
(`slots`) and plain REPS uses the unbounded FIFO (`fifo`). Neither algorithm
populates the other's column, so exactly one of the two is meaningful in any
given trace.
...
- `--verify` checks the trace invariants (buffer-geometry consistency, pop/draw
  ...
  skipped and `--verify` says so. On traces carrying a `fifo` column the FIFO
  invariants run instead: a `fresh_pop` draw must take the current front and
  leave the tail untouched, and an ACK that grows the FIFO must append exactly
  the acked EV at the back. `--verify` names which set it ran.
...
- **Buffer grid** — one cell per slot; `↑` marks the write head, `fz ↓` marks the
  ...
  Green = valid (usable) EV; grey = stale — consuming an EV only clears `isValid`,
  the cached value stays in the slot until a later ACK overwrites it
  (`remove_earliest_fresh()`, `buffer_reps.cpp`); dashed = slot never written.
  Hidden on traces whose algorithm does not drive this buffer.
- **FIFO grid** — shown only on traces carrying a `fifo` column, for algorithms
  that recycle through `_next_pathid`. Front-first: the leftmost cell (blue ring)
  is the next EV to be reused; a clean ACK appends on the right. A dashed `+N`
  cell marks entries elided past the 128-entry logging cap, and `fifoDepth` in
  the state panel reports the true depth including them. Exactly one of the
  buffer grid / FIFO grid is shown per trace.
...
- **Cause cross-reference** — `FREEZE` links to the outstanding SEND/RTX whose RTO
  produced it (matched on the EV the RTO recorded, skipping candidates already
  ACKed/NACKed by then); `UNFREEZE` links to the FREEZE it thaws, with the frozen
  duration; `RTX` links to the NACK for its own PSN, or — for a `frozen_pop`
  replay with no NACK — names the frozen epoch it belongs to, explicitly *not*
  as its trigger. Note many RTX rows can legitimately point at one FREEZE: every
  packet outstanding when the buffer froze is replayed, not just the one that
  timed out.
