...
# `fifo` was appended to the schema after the first traces were captured, so
# both widths are accepted: a 17-column file simply has no FIFO information
# (parsed as None, rendered as "not instrumented"), while an 18-column file
# carries the unbounded _next_pathid contents that plain REPS/MIXED recycle
# through. Everything before `fifo` is byte-identical between the two.
FIFO_COLUMN = "fifo"
EXPECTED_COLUMNS_WITH_FIFO = EXPECTED_COLUMNS + [FIFO_COLUMN]
...
def parse_fifo(raw, line_no):
    """Parse the `fifo` field: the unbounded _next_pathid FIFO, front first.

    "-" means this LB algorithm does not use the FIFO at all (returns None);
    "" means it does but the FIFO is currently empty (returns []). A trailing
    "+N" token marks entries elided by the writer's row cap and is preserved as
    a sentinel so the viewer can say so rather than under-reporting depth.
    """
    if raw == "-":
        return None
    if raw == "":
    ...
    for i, chunk in enumerate(raw.split("|")):
        if chunk.startswith("+"):
            out.append({"elided": _parse_int(chunk[1:], line_no, f"fifo[{i}].elided")})
            continue
        out.append({"ev": _parse_int(chunk, line_no, f"fifo[{i}]")})
    return out


def parse_row(row, line_no):
    if len(row) not in (len(EXPECTED_COLUMNS), len(EXPECTED_COLUMNS_WITH_FIFO)):
        raise EventParseError(
            f"line {line_no}: expected {len(EXPECTED_COLUMNS)} or "
            f"{len(EXPECTED_COLUMNS_WITH_FIFO)} fields, got {len(row)}"
        )
    columns = (EXPECTED_COLUMNS_WITH_FIFO
               if len(row) == len(EXPECTED_COLUMNS_WITH_FIFO) else EXPECTED_COLUMNS)
    ...
    event = {
        ...
        # None when the column is absent (pre-fifo trace) or "-" (algo has no FIFO).
        "fifo": parse_fifo(d[FIFO_COLUMN], line_no) if FIFO_COLUMN in d else None,
    }
...
def load_events(path):
    ...
    with open(path, "r", newline="", encoding="utf-8") as f:
        ...
        try:
            ...
            return events
        if header not in (EXPECTED_COLUMNS, EXPECTED_COLUMNS_WITH_FIFO):
            raise EventParseError(
                f"line 1: unexpected header {header!r}, expected "
                f"{EXPECTED_COLUMNS!r} or {EXPECTED_COLUMNS_WITH_FIFO!r}"
            )
...
def verify_events(events):
    ...
    for e in events:
        ...
        # ===== ADDED (reps-fifo-trace) =====
        # _next_pathid invariants (plain REPS / MIXED). Skipped for rows whose
        # FIFO was elided by the writer's cap, since the tail is unknown there.
        prev_e = prev_event_by_src.get(src)
        prev_fifo = prev_e["fifo"] if prev_e else None
        cur_fifo = e["fifo"]
        complete = (cur_fifo is not None and prev_fifo is not None
                    and not any("elided" in x for x in cur_fifo)
                    and not any("elided" in x for x in prev_fifo))
        if complete:
            if kind in ("SEND", "RTX", "RTS") and ev_src == "fresh_pop":
                # pop_front: the EV sent must be the old front, and the rest
                # of the queue must be unchanged behind it.
                if not prev_fifo:
                    violations.append(
                        f"row n={n}: {kind} ev_src=fresh_pop but FIFO was empty"
                    )
                elif prev_fifo[0]["ev"] != e["ev"]:
                    violations.append(
                        f"row n={n}: {kind} ev_src=fresh_pop EV {e['ev']} != FIFO front "
                        f"{prev_fifo[0]['ev']}"
                    )
                elif cur_fifo != prev_fifo[1:]:
                    violations.append(
                        f"row n={n}: {kind} ev_src=fresh_pop but FIFO tail changed "
                        f"beyond the popped front"
                    )
            elif kind == "ACK" and len(cur_fifo) == len(prev_fifo) + 1:
                # push_back on PATH_GOOD: only ever appends the acked EV.
                if cur_fifo[:-1] != prev_fifo:
                    violations.append(
                        f"row n={n}: ACK grew the FIFO but did not append at the back"
                    )
                elif cur_fifo[-1]["ev"] != e["ev"]:
                    violations.append(
                        f"row n={n}: ACK appended EV {cur_fifo[-1]['ev']} != acked EV {e['ev']}"
                    )
        # ===== END ADDED (reps-fifo-trace) =====
...
.cell.valid { background: var(--green); border-color: var(--green); color: #052e16; }
/* FIFO cells are narrower than circular-buffer slots: the list can hold well
   over a hundred entries, and its index carries no meaning beyond order. */
.cell.fifo { width: 44px; height: 40px; font-size: 14px; background: var(--green);
  border-color: var(--green); color: #052e16; }
.cell.fifo.front { box-shadow: 0 0 0 3px var(--blue); }
.cell.fifo.elided { background: var(--sunken); border-color: var(--border);
  border-style: dashed; color: var(--muted); font-size: 11px; width: 56px; }
.cell.stale { background: var(--sunken); border-color: var(--border); color: var(--muted); }
...
.legend summary { cursor: pointer; font-size: 12px; color: var(--muted); user-select: none; }
.legend summary:hover { color: var(--ink); }
.legend-body { margin-top: 8px; display: grid; grid-template-columns: 100px 1fr; gap: 6px 10px; font-size: 12.5px; }
.legend-body .tag { min-width: 76px; }
.legend-body .desc { color: var(--ink); }
</style>
</head>
<body>
<div class="wrap">
  <div class="card">
    ...
    <details class="legend" open>
      <summary>Legend &amp; event explanations</summary>
      <div class="legend-body">
        <span class="tag tag-SEND">SEND</span><span class="desc">New data packet sent, with its drawn EV.</span>
        <span class="tag tag-RTX">RTX</span><span class="desc">Retransmit of a lost/expired packet, fresh EV drawn.</span>
        <span class="tag tag-RTS">RTS</span><span class="desc">Request-To-Send, restarts a stalled flow (max 1/RTT).</span>
        <span class="tag tag-ACK">ACK</span><span class="desc">Ack received, clean (no ECN mark).</span>
        <span class="tag tag-ACK ecn1">ACK</span><span class="desc">Red = ECN-marked.</span>
        <span class="tag tag-NACK">NACK</span><span class="desc">Switch trimmed/dropped packet; seqno requeued for RTX.</span>
        <span class="tag tag-FREEZE">FREEZE</span><span class="desc">Buffer freezes on RTO (no ACK/NACK in time). EV = trigger.</span>
        <span class="tag tag-UNFREEZE">UNFREEZE</span><span class="desc">Buffer thaws (timer or fresh good EV); buffer reset.</span>
      </div>
    </details>
    <div class="row">
    ...
    <div class="hint" id="bufferLegend" style="margin-bottom:4px;"></div>
    <div class="buffer-grid" id="bufferGrid"></div>
    <div class="hint" id="fifoLegend" style="margin-bottom:4px;"></div>
    <div class="buffer-grid" id="fifoGrid"></div>
...
// ===== ADDED (reps-event-crossref): FREEZE/UNFREEZE/RTX cause links =====
// EV index: "<src_id>:<ev>" -> sorted row indices of SEND/RTX draws of that EV.
// Used to find which outstanding packet's RTO triggered a given FREEZE
// (FREEZE.ev == UecSrc::rtxTimerExpired's rto_trigger_ev == the timed-out
// packet's sent_ev, per uec.cpp ~line 3994 - it's the packet, not the buffer
// head at RTO time).
const evIndex = {};
EVENTS.forEach((e, i) => {
  if (!["SEND","RTX"].includes(e.event)) return;
  const key = e.src_id + ":" + e.ev;
  (evIndex[key] = evIndex[key] || []).push(i);
});
// NACK index: "<src_id>:<seqno>" -> sorted row indices of NACKs for that PSN.
const nackIndex = {};
EVENTS.forEach((e, i) => {
  if (e.event !== "NACK" || e.seqno === -1) return;
  const key = e.src_id + ":" + e.seqno;
  (nackIndex[key] = nackIndex[key] || []).push(i);
});
// FREEZE index: src_id -> sorted row indices of FREEZE events for that src.
const freezeIndex = {};
EVENTS.forEach((e, i) => {
  if (e.event !== "FREEZE") return;
  (freezeIndex[e.src_id] = freezeIndex[e.src_id] || []).push(i);
});
// Was PSN `seqno` (same src) ACKed or NACKed strictly between rows
// (fromIdx, toIdx)? If so, a SEND/RTX of it can't be the packet still
// outstanding at `toIdx` - some later resend is the real culprit.
function ackedOrNackedBetween(src, seqno, fromIdx, toIdx) {
  for (let i = fromIdx + 1; i < toIdx; i++) {
    const e = EVENTS[i];
    if (e.src_id === src && e.seqno === seqno && (e.event === "ACK" || e.event === "NACK")) return true;
  }
  return false;
}
// The packet whose RTO produced this FREEZE.
// rtxTimerExpired() (uec.cpp) always services _send_times.begin() - the
// EARLIEST outstanding send - and reports that packet's sent_ev as
// rto_trigger_ev. So when several sends of the same EV are still unacked at
// freeze time, the trigger is the OLDEST of them, not the most recent: scan
// oldest-to-newest and take the first still-outstanding candidate.
function findTriggerForFreeze(e, beforeIdx) {
  const list = evIndex[e.src_id + ":" + e.ev];
  if (!list) return null;
  for (const i of list) {
    if (i >= beforeIdx) break;
    const cand = EVENTS[i];
    if (!ackedOrNackedBetween(cand.src_id, cand.seqno, i, beforeIdx)) return i;
  }
  return null;
}
// Nearest previous FREEZE (same src) strictly before beforeIdx - the freeze
// epoch a given UNFREEZE thaws, or (for RTX) the epoch a frozen_pop replay
// belongs to.
function findFreezeBefore(e, beforeIdx) {
  const list = freezeIndex[e.src_id];
  if (!list) return null;
  let best = null;
  for (const i of list) { if (i < beforeIdx) best = i; else break; }
  return best;
}
// Nearest previous NACK for this RTX's own PSN (same src), strictly before
// beforeIdx - the fast-retransmit trigger.
function findNackForRtx(e, beforeIdx) {
  if (e.seqno === -1) return null;
  const list = nackIndex[e.src_id + ":" + e.seqno];
  if (!list) return null;
  let best = null;
  for (const i of list) { if (i < beforeIdx) best = i; else break; }
  return best;
}
// ===== END ADDED (reps-event-crossref) =====
...
// ===== ADDED (reps-event-crossref): buffer legend depends on whether this LB
// algorithm drives the *circular* buffer (mirrors buffer_is_live() in the
// Python tool). Only FREEZING / FREEZING_PXR populate circular_buffer_reps,
// which is what the slots column snapshots. Plain REPS still keeps an EV
// buffer - the unbounded FIFO _next_pathid (uec.h:947, push_back on clean ACK
// / pop_front on reuse) - it is simply not instrumented by this trace, so its
// slots/fresh/buf_size columns are all zero. Saying "no buffer" there would be
// wrong; saying nothing would read as a broken viewer. =====
const bufferLive = EVENTS.some(e => e.slots.some(s => s.valid));
// fifo === null on every row means either a pre-fifo-column trace or an
// algorithm that does not use _next_pathid at all.
const fifoLive = EVENTS.some(e => e.fifo !== null);
document.getElementById("bufferLegend").innerHTML = bufferLive
  ? 'Circular buffer: <span style="color:var(--green);font-weight:700;">green</span> = valid EV (usable) &middot; ' +
    '<span style="color:var(--muted);font-weight:700;">grey</span> = stale (consumed, value kept until overwritten) &middot; ' +
    'dashed = slot never written'
  : (fifoLive
      ? 'Circular buffer: unused by this algorithm - it recycles EVs through the FIFO below.'
      : 'Circular buffer: this trace snapshots the bounded circular buffer, which only FREEZING drives. ' +
        'Plain REPS recycles EVs through an unbounded FIFO (<code>_next_pathid</code>) that this ' +
        'trace predates, so these cells stay dashed - the algorithm does have a buffer, this ' +
        'older trace just cannot see it.');
document.getElementById("bufferGrid").style.display = bufferLive ? "" : "none";
document.getElementById("fifoLegend").innerHTML = fifoLive
  ? 'EV FIFO (<code>_next_pathid</code>, unbounded): front-first - leftmost is the next EV to be ' +
    'reused (<code>pop_front</code>); a clean ACK appends on the right (<code>push_back</code>). ' +
    '<span style="color:var(--blue);font-weight:700;">Blue ring</span> = front.'
  : "";
document.getElementById("fifoLegend").style.display = fifoLive ? "" : "none";
document.getElementById("fifoGrid").style.display = fifoLive ? "" : "none";
// ===== END ADDED (reps-event-crossref) =====
...
function renderState(e, prevE) {
  ...
  // ===== ADDED (reps-fifo-trace): FIFO depth, only for algorithms that use it =====
  if (e.fifo !== null) {
    const elided = e.fifo.reduce((a, x) => a + (x.elided || 0), 0);
    const logged = e.fifo.length - (elided ? 1 : 0);
    fields.push(["fifoDepth", logged + elided]);
  }
  // ===== END ADDED (reps-fifo-trace) =====
  ...
  fields.forEach(([k, v]) => {
    ...
    if (prevE) {
      const pv = {head: prevE.head, frozenHead: prevE.frozen_head, fresh: prevE.fresh,
                  ...
                  frozenEv: prevE.frozen_ev,
                  // ===== ADDED (reps-fifo-trace) =====
                  fifoDepth: prevE.fifo === null ? undefined
                    : prevE.fifo.reduce((a, x) => a + (x.elided || 1), 0)}[k];
      if (pv !== undefined && pv !== v) div.className += " changed";
    }
  ...
  e.slots.forEach((s, i) => {
    ...
    // A slot's value is NOT cleared when it's consumed - only isValid flips
    // to false (paper Algorithm 2 L3-5 / buffer_reps.cpp remove_earliest_fresh):
    // the EV stays cached until a later ACK overwrites it. lifetime>0 (or
    // valid) means the slot has been written at least once, so its stale
    // value is still worth showing; lifetime==0 && !valid means never written.
    const everWritten = s.valid || s.lifetime > 0;
    c.className = "cell" + (s.valid ? " valid" : (everWritten ? " stale" : " invalid"));
    ...
    c.textContent = everWritten ? s.value : "·";
    bufGrid.appendChild(c);
  });

  // ===== ADDED (reps-fifo-trace) =====
  // The FIFO has no fixed geometry: entries shift left on pop_front, so a
  // positional diff against the previous row would flash almost every cell.
  // Only the front is marked, plus the writer's overflow sentinel.
  const fifoGrid = document.getElementById("fifoGrid");
  if (fifoGrid) {
    fifoGrid.innerHTML = "";
    if (e.fifo === null) {
      // Row carries no FIFO information; leave the grid empty rather than
      // rendering a misleading "empty FIFO".
    } else if (e.fifo.length === 0) {
      const c = document.createElement("div");
      c.className = "cell fifo elided";
      c.textContent = "empty";
      fifoGrid.appendChild(c);
    } else {
      e.fifo.forEach((entry, i) => {
        const c = document.createElement("div");
        if (entry.elided !== undefined) {
          c.className = "cell fifo elided";
          c.textContent = "+" + entry.elided;
          c.title = entry.elided + " further entries not logged (writer row cap)";
        } else {
          c.className = "cell fifo" + (i === 0 ? " front" : "");
          c.textContent = entry.ev;
        }
        fifoGrid.appendChild(c);
      });
    }
...
function render() {
  ...
  psnLink.innerHTML = "";
  function jumpLink(text, matchIdx) {
  ...
  if ((e.event === "ACK" || e.event === "NACK") && e.seqno !== -1) {
    ...
    if (matchIdx !== null) {
      ...
      jumpLink((e.event === "ACK" ? "↑ acks" : "↑ refers to") +
        " PSN " + e.seqno + " sent at row " + (matchIdx + 1) + " (t=" +
        EVENTS[matchIdx].time_ns.toFixed(1) + " ns, RTT=" + rtt.toFixed(1) +
        " ns) — click to jump", matchIdx);
    } else {
      psnLink.textContent = "PSN " + e.seqno + ": no matching SEND/RTX row in this trace";
    }
  // ===== ADDED (reps-event-crossref) =====
  } else if (e.event === "FREEZE") {
    const matchIdx = findTriggerForFreeze(e, idx);
    if (matchIdx !== null) {
      const outstanding = e.time_ns - EVENTS[matchIdx].time_ns;
      jumpLink("↑ triggered by EV " + e.ev + " packet (PSN " + EVENTS[matchIdx].seqno +
        ") sent at row " + (matchIdx + 1) + " (t=" + EVENTS[matchIdx].time_ns.toFixed(1) +
        " ns, outstanding " + outstanding.toFixed(1) + " ns before RTO) — click to jump", matchIdx);
    } else {
      psnLink.textContent = "FREEZE ev=" + e.ev + ": no unacked SEND/RTX of this EV found before it";
    }
  } else if (e.event === "UNFREEZE") {
    const matchIdx = findFreezeBefore(e, idx);
    if (matchIdx !== null) {
      const duration = e.time_ns - EVENTS[matchIdx].time_ns;
      jumpLink("↑ thaws FREEZE at row " + (matchIdx + 1) + " (t=" +
        EVENTS[matchIdx].time_ns.toFixed(1) + " ns, frozen for " + duration.toFixed(1) +
        " ns) — click to jump", matchIdx);
    } else {
      psnLink.textContent = "UNFREEZE: no prior FREEZE found for this src";
    }
  } else if (e.event === "RTX") {
    const nackIdx = findNackForRtx(e, idx);
    if (nackIdx !== null) {
      const delay = e.time_ns - EVENTS[nackIdx].time_ns;
      jumpLink("↑ fast-retransmit of NACK at row " + (nackIdx + 1) + " (t=" +
        EVENTS[nackIdx].time_ns.toFixed(1) + " ns, " + delay.toFixed(1) +
        " ns later) — click to jump", nackIdx);
    } else {
      // No NACK for this PSN => the retransmit was driven by that packet's own
      // RTO. The useful target is the send that timed out, not the FREEZE row:
      // a FREEZE is logged only for the RTO that *entered* frozen mode, and
      // rtxTimerExpired() guards the whole freeze block on !isFrozenMode(), so
      // every later RTO retransmits without re-freezing or resetting the thaw
      // timer. Pointing those rows at the FREEZE would imply a causal link that
      // does not exist.
      const sendIdx = findSendForPsn(e, idx);
      if (sendIdx !== null) {
        const outstanding = e.time_ns - EVENTS[sendIdx].time_ns;
        const freezeIdx = findFreezeBefore(e, idx);
        const isFreezeTrigger = freezeIdx !== null &&
          EVENTS[freezeIdx].ev === EVENTS[sendIdx].ev &&
          findTriggerForFreeze(EVENTS[freezeIdx], freezeIdx) === sendIdx;
        let note;
        if (e.frozen_mode === 1 && !isFreezeTrigger) {
          note = " — RTO while already in freezing mode: the packet is resent on a " +
            "fresh EV, but this does not re-enter freezing or reset the thaw timer" +
            (freezeIdx !== null
              ? " (epoch began at row " + (freezeIdx + 1) + ")" : "") + ".";
        } else if (isFreezeTrigger) {
          note = " — this is the RTO that entered freezing mode (FREEZE at row " +
            (freezeIdx + 1) + ").";
        } else {
          note = " — RTO in normal mode.";
        }
        jumpLink("↑ RTO retransmit of PSN " + e.seqno + " sent at row " +
          (sendIdx + 1) + " (t=" + EVENTS[sendIdx].time_ns.toFixed(1) +
          " ns, EV " + EVENTS[sendIdx].ev + ", outstanding " +
          outstanding.toFixed(1) + " ns)" + note + " Click to jump.", sendIdx);
      } else {
        psnLink.textContent = "RTX PSN " + e.seqno +
          ": no NACK and no earlier send of this PSN in this trace";
      }
    }
  }
  // ===== END ADDED (reps-event-crossref) =====
...
def main(argv=None):
    ...
    if args.verify:
        ...
        elif any(e["fifo"] is not None for e in events):
            print(f"VERIFY OK: {len(events)} events, ordering/fresh and FIFO "
                  f"(_next_pathid pop_front/push_back) invariants pass "
                  f"(circular buffer unused by this algorithm, so its geometry "
                  f"invariants were skipped)")
        else:
