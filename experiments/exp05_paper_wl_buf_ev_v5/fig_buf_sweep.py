#!/usr/bin/env python3
"""
fig_buf_sweep.py — exp05 Part A: REPS FREEZING buffer size sweep.

Paper workloads (perm_n128_s8388608.cm, test_symm16.cm), 2-tier 128-host topology.
Follows the paper's artifact script style (fig_12_evs_cc.py, fig_8_extreme_failures.py).

Produces 5 figures:
  A1a  FCT CDF per buffer size — perm_n128   (like fig_12)
  A1b  FCT CDF per buffer size — symm16      (like fig_12)
  A2   Max FCT vs buffer size  — both wl     (like fig_8)
  A3   Port utilisation per uplink — symm16  (like fig_1, routing efficiency)
  A4   cwnd time-series per buffer size      (CC behaviour, exp04-A6 style)
"""

import os, re, csv as csv_module, warnings
...
for d in [DATA_DIR/"fct_A", DATA_DIR/"cwnd_A", DATA_DIR/"portutil_A", PLOT_DIR]:
...
PKT_BYTES_UTIL  = 4096         # packet size for port-util Gbps calculation (matching fig_1)
BIN_NS          = 20_000       # 20 µs bins for port utilisation (matching fig_1)

# Paper colour palette (matching fig_12, fig_8)
...
COMMON = (
    ...
    f"-load_balancing_algo freezing -disable_tor_ecn"
)
...
def parse_cwnd_file(path):
    """Return (time_us[], cwnd_pkts[]) from reps_state.csv."""
    times, cwnds = [], []
    try:
        with open(path) as f:
...
def parse_port_util(save_dir):
    """
    Parse portSwitch_LowerPod_0_.txt from a -save_data_folder directory.
    Returns dict: port_id -> (time_us[], gbps[]).
    Follows the same binning logic as fig_1_symmetric_micro.py.
    """
    port_file = Path(save_dir) / "port" / "portSwitch_LowerPod_0_.txt"
    pattern = re.compile(r"LS\d+->US(\d+)")
    bins = defaultdict(lambda: defaultdict(int))  # bin_ns -> port -> count
    ...
    try:
        with open(port_file) as f:
            for line in f:
                ...
                if m:
                    port = int(m.group(1))
                    bin_key = (ts_ns // BIN_NS) * BIN_NS
                    bins[bin_key][port] += 1
    except FileNotFoundError:
        return {}

    # Convert to per-port timeseries
    result = defaultdict(list)
    for bin_ns in sorted(bins):
        for port, count in bins[bin_ns].items():
            gbps = count * PKT_BYTES_UTIL * 8 / BIN_NS  # bytes/ns = Gbps
            result[port].append((bin_ns / 1000.0, gbps))  # ns → µs
    return {port: (np.array([t for t, _ in v]), np.array([g for _, g in v]))
            for port, v in result.items()}

# ── Build commands ─────────────────────────────────────────────────────────────
fct_cmds, cwnd_cmds, portutil_cmds = [], [], []

# FCT: all buf × workload × sev × seed
for buf in BUF_VALUES:
    for wl, cm in [("perm_n128", PERM_CM), ("symm16", SYMM_CM)]:
        for sev in SEVS:
            for seed in SEEDS:
                out = DATA_DIR / "fct_A" / f"buf{buf}_{wl}_sev{sev}_s{seed}.out"
                if out.exists():
                    continue
                fct_cmds.append(
                    f"{HTSIM} {COMMON} -reps_buffer_size {buf} -paths 65535 "
                    f"-tm {cm} -seed {seed} {fail_flags(sev)} > {out} 2>&1"
                )

# cwnd: perm_n128, sev=1, seed=42 per buffer — for A4
for buf in BUF_VALUES:
    d = DATA_DIR / "cwnd_A" / f"buf{buf}"
...
# Port util: symm16, seed=42, both sev — for A3
for buf in BUF_VALUES:
    for sev in SEVS:
        save_dir = DATA_DIR / "portutil_A" / f"buf{buf}_sev{sev}"
...
# ══════════════════════════════════════════════════════════════════════════════
# A1: FCT CDF per buffer size  (like fig_12)
# ══════════════════════════════════════════════════════════════════════════════
print("\n=== Plotting ===")
for wl_name, wl_size in [("perm_n128", FLOW_SIZE_PERM), ("symm16", FLOW_SIZE_SYMM)]:
    ...
    for ax, sev in zip(axes, SEVS):
        for i, (buf, label) in enumerate(zip(BUF_VALUES, BUF_LABELS)):
            all_fcts = []
            for seed in SEEDS:
                out = DATA_DIR / "fct_A" / f"buf{buf}_{wl_name}_sev{sev}_s{seed}.out"
        ...
        sev_label = "No failure" if sev == 0 else "Sev=1 (link fail t=50–200 µs)"
        ax.set_title(sev_label, fontsize=10)
        ax.grid(True, linestyle=':', linewidth=0.75, alpha=0.75)
        ax.set_ylim(0, 1.05)

    legend_elems = [
        Line2D([0],[0], color=DARK2[i], marker=MARKERS[i], lw=2, label=f"buf={l}")
        for i, l in enumerate(BUF_LABELS)
    ...
    fig.suptitle(f"FCT CDF — Buffer Sweep  ({wl_name})", fontsize=11)
    fig.tight_layout()
    tag = f"A1_fct_cdf_buf_{wl_name}"
    for ext in (".png", ".pdf"):
...
# ══════════════════════════════════════════════════════════════════════════════
# A2: Max FCT vs buffer size  (like fig_8)
# ══════════════════════════════════════════════════════════════════════════════
...
for ax, (wl_name, wl_size) in zip(axes, [("perm_n128", FLOW_SIZE_PERM), ("symm16", FLOW_SIZE_SYMM)]):
    ...
    for sev in SEVS:
        ...
        for i, (mf, buf) in enumerate(zip(max_means, BUF_VALUES)):
            if np.isnan(mf):
                continue
    ...
    ax.plot(range(len(BUF_VALUES)), [ideal] * len(BUF_VALUES),
            color='gray', linestyle=':', linewidth=1.5,
            label=f"ideal ({ideal:.0f} µs)")
    ax.set_xticks(range(len(BUF_VALUES)))
    ax.set_xticklabels(BUF_LABELS)
    ax.set_xlabel("REPS Buffer Size", fontsize=11)
    ax.set_ylabel("Max FCT (µs)", fontsize=11)
    ax.set_title(wl_name, fontsize=11)
    ax.legend(fontsize=8, frameon=False)
    ax.grid(True, linestyle=':', linewidth=0.75, alpha=0.75)

fig.suptitle("Max FCT vs Buffer Size", fontsize=12)
fig.tight_layout()
for ext in (".png", ".pdf"):
    fig.savefig(str(PLOT_DIR / ("A2_max_fct_vs_buf" + ext)), dpi=300, bbox_inches='tight')
plt.close(fig)
print("  Saved A2_max_fct_vs_buf.png")

# ══════════════════════════════════════════════════════════════════════════════
# A3: Port utilisation per uplink  (like fig_1 — routing efficiency)
# Show buf=∞ vs buf=1 as 2-row × 2-col grid (sev=0 left, sev=1 right)
# ══════════════════════════════════════════════════════════════════════════════
UPLINK_CM = plt.cm.get_cmap('tab10', 8)

fig, axes = plt.subplots(2, 2, figsize=(10, 5.5))

for row, buf in enumerate([1024, 1]):
    buf_label = "∞" if buf == 1024 else str(buf)
    for col, sev in enumerate(SEVS):
        ...
        ax.set_title(f"buf={buf_label},  {fail_str}", fontsize=10)
        ax.set_xlim(0, 700)   # first 700 µs covers full transfer window
        ax.set_ylim(0, 450)
...
fig.suptitle(
    ...
    "Top row: buf=∞ (full diversity)   Bottom row: buf=1 (single-path reuse)",
    fontsize=10,
)
fig.tight_layout(rect=[0, 0, 1, 0.93])
for ext in (".png", ".pdf"):
    fig.savefig(str(PLOT_DIR / ("A3_port_util_buf" + ext)), dpi=300, bbox_inches='tight')
plt.close(fig)
print("  Saved A3_port_util_buf.png")

# ══════════════════════════════════════════════════════════════════════════════
# A4: cwnd time-series per buffer size  (CC behaviour)
# ══════════════════════════════════════════════════════════════════════════════
fig, ax = plt.subplots(1, 1, figsize=(7, 3.2))

for i, (buf, label) in enumerate(zip(BUF_VALUES, BUF_LABELS)):
    state_path = DATA_DIR / "cwnd_A" / f"buf{buf}" / "reps_state.csv"
    times, cwnds = parse_cwnd_file(state_path)
    if len(times) == 0:
        continue
    ax.plot(times, cwnds, color=DARK2[i % len(DARK2)], linewidth=1.5,
            label=f"buf={label}", alpha=0.9)
...
ax.set_title(
    "CC Window Time-Series — Buffer Sweep\n"
...
fig.tight_layout()
for ext in (".png", ".pdf"):
    fig.savefig(str(PLOT_DIR / ("A4_cwnd_timeseries_buf" + ext)), dpi=300, bbox_inches='tight')
plt.close(fig)
print("  Saved A4_cwnd_timeseries_buf.png")

print(f"\nAll Part A plots saved to {PLOT_DIR}")
