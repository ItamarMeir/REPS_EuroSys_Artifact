...
`bash experiments/test_srv6/scripts/run_tests.sh`
...
python3 -c "import pandas as pd; df = pd.read_csv('experiments/test_srv6/runs/T10_k16_reachability.csv'); print('unique ack_ev:', df['ack_ev'].nunique(), 'range:', df['ack_ev'].min(), '-', df['ack_ev'].max())"
```
